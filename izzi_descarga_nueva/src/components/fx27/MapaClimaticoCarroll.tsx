...
export const MapaClimaticoCarroll = ({ onClose, unidades = [] }: MapaClimaticoCarrollProps) => {
  ...
  const obtenerAPIKey = async () => {
    try {
      // Obtener Google Maps API Key para el mapa base
      ...
      if (!gmapsResponse.ok) {
        throw new Error('Error obteniendo Google Maps API Key');
      }

      const gmapsData = await gmapsResponse.json();
      setApiKey(gmapsData.apiKey);
    } catch (err) {
  ...
  const cambiarCapa = async (nuevaCapa: CapaClimatica) => {
    if (!map) return;
    ...
    try {
      const owmResponse = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-d84b50bb/api-keys/openweather`,
        {
          headers: {
            'Authorization': `Bearer ${publicAnonKey}`
          }
      ...
      if (!owmResponse.ok) {
        console.error('Error obteniendo OpenWeatherMap API Key');
        alert('⚠️ Configura OPENWEATHER_API_KEY en Supabase (ver /docs/MAPA_CLIMATICO_INSTRUCCIONES.md)');
        return;
      }

      const owmData = await owmResponse.json();
      const owmApiKey = owmData.apiKey;
      ...
      const nuevaCapaWeather = new google.maps.ImageMapType({
        getTileUrl: (coord, zoom) => {
          return `https://tile.openweathermap.org/map/${tipoOWM}/${zoom}/${coord.x}/${coord.y}.png?appid=${owmApiKey}`;
        },
      ...
      console.error('Error configurando capa meteorológica:', error);
      alert('⚠️ Error al cargar la capa meteorológica. Ver consola para detalles.');
    }
  };
