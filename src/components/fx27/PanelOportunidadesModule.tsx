...
export const PanelOportunidadesModule = ({ onBack }: PanelOportunidadesModuleProps) => {
  ...
  const extraerTextoPDF = async (file: File): Promise<string> => {
    ...
    let texto = '';
    for (let i = 1; i <= pdf.numPages; i++) { const page = await pdf.getPage(i); const content = await page.getTextContent(); texto += content.items.map((item: any) => item.str).join(' ') + '\n'; }
    return texto;
  };

  const analizarCotizacion = async (pdfText: string): Promise<any> => {
    try {
      const response = await fetch(`https://${projectId}.supabase.co/functions/v1/analyze-cotizacion`, { method: 'POST', headers: { 'Authorization': `Bearer ${publicAnonKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ pdfText, tipoCambio }) });
      const result = await response.json();
  ...
  return (
    <ModuleTemplate title="Panel de Oportunidades" onBack={onBack} headerImage={MODULE_IMAGES.PANEL_OPORTUNIDADES}>
      <div className="flex flex-col h-[calc(100vh-120px)]">
        {/* HEADER FIJO - Búsqueda y filtros */}
        ...
        <div className="flex-1 mx-4 mb-4 rounded-2xl bg-[var(--fx-surface)] border border-white/10 overflow-hidden flex flex-col">
          ...
          {/* Body con scroll - filas completas */}
              ...
              <tbody>
                {filteredLeads.length === 0 ? (
                  ...
                  filteredLeads.map((lead, index) => (
                    <tr key={lead.id} className={`border-b border-white/5 hover:bg-white/5 transition-colors ${lead.eliminado ? 'opacity-50 bg-red-500/5' : ''}`} style={{ height: '48px', scrollSnapAlign: 'start' }}>
                      ...
                      <td className="px-2 py-2 text-white" style={{ fontSize: '11px', fontWeight: 700, width: '13%' }}>{lead.nombreEmpresa}{lead.eliminado && <span className="ml-1 text-red-400 text-xs">(X)</span>}</td>
                      ...
                      <td className="px-2 py-2" style={{ width: '10%' }}><div className="flex flex-wrap gap-0.5">{lead.tipoServicio.slice(0,2).map((t, i) => <span key={i} className="px-1.5 py-0.5 rounded bg-blue-500/20 text-blue-400" style={{ fontSize: '9px', fontWeight: 600 }}>{t}</span>)}{lead.tipoServicio.length > 2 && <span className="text-blue-400" style={{ fontSize: '9px' }}>+{lead.tipoServicio.length - 2}</span>}</div></td>
                      <td className="px-1.5 py-2" style={{ width: '10%' }}><div className="flex flex-wrap gap-0.5">{lead.tipoViaje.slice(0,2).map((t, i) => <span key={i} className="px-1.5 py-0.5 rounded bg-green-500/20 text-green-400" style={{ fontSize: '9px', fontWeight: 600 }}>{t}</span>)}{lead.tipoViaje.length > 2 && <span className="text-green-400" style={{ fontSize: '9px' }}>+{lead.tipoViaje.length - 2}</span>}</div></td>
                      ...
                      <td className="px-2 py-2" style={{ width: '8%' }}><div className="flex flex-col"><span className="text-white" style={{ fontSize: '10px', fontWeight: 600 }}>{new Date(lead.fechaCreacion).toLocaleDateString('es-MX')}</span>{lead.fechaActualizacion && <span className="text-yellow-400" style={{ fontSize: '9px' }}>Act: {new Date(lead.fechaActualizacion).toLocaleDateString('es-MX')}</span>}</div></td>
      ...
      {selectedLead && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setSelectedLead(null)}>
          <div className="bg-[var(--fx-surface)] rounded-2xl border border-white/20 w-[95vw] max-w-[1400px] max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="sticky top-0 bg-[var(--fx-surface)] border-b border-white/10 p-4 flex items-center justify-between"><h3 className="text-white flex items-center gap-3" style={{ fontFamily: "'Exo 2', sans-serif", fontSize: '22px', fontWeight: 700 }}><Building2 className="w-6 h-6 text-blue-400" />{selectedLead.nombreEmpresa}</h3><button onClick={() => setSelectedLead(null)} className="p-2 rounded-lg hover:bg-white/10"><X className="w-6 h-6 text-white" /></button></div>
            <div className="p-6">
              <div className="grid grid-cols-4 gap-4">
                ...
                <div className="p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/20">
                  ...
                  {(!selectedLead.cotizaciones || !selectedLead.cotizaciones.filter(c => !c.eliminado).length) ? <div className="text-center py-4 text-[var(--fx-muted)] text-sm">Sin cotizaciones</div> : <div className="space-y-2 max-h-[200px] overflow-y-auto">{selectedLead.cotizaciones.filter(c => !c.eliminado).map((cot, i) => <div key={i} className="flex items-center justify-between p-2 rounded-lg bg-white/5 border border-white/10"><div className="flex items-center gap-2 flex-1 min-w-0"><FileText className="w-4 h-4 text-emerald-400 flex-shrink-0" /><div className="truncate text-white text-xs">{cot.nombre}</div></div><a href={cot.url} download={cot.nombre} className="p-1.5 rounded bg-blue-500/20 text-blue-400 flex-shrink-0"><Download className="w-3 h-3" /></a></div>)}</div>}
                </div>
              </div>
      ...
      {deleteModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => { setDeleteModal(null); setDeleteConfirmText(''); }}>
          <div className="bg-[var(--fx-surface)] rounded-2xl border border-red-500/30 w-[500px] p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mb-4 p-4 rounded-lg bg-red-500/10 border border-red-500/20"><p className="text-white mb-2">¿Eliminar el lead?</p><p className="text-white font-bold text-lg">{deleteModal.nombreEmpresa}</p><p className="text-[var(--fx-muted)] text-sm mt-1">{deleteModal.nombreContacto} - {deleteModal.correoElectronico}</p></div>
      ...
      {cotizacionesModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setCotizacionesModal(null)}>
          <div className="bg-[var(--fx-surface)] rounded-2xl border border-white/20 w-[900px] max-h-[90vh] overflow-y-auto p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mb-6"><label className={`flex items-center justify-center gap-2 px-4 py-4 rounded-xl ${analizando ? 'bg-blue-600/50' : 'bg-blue-600 hover:bg-blue-700'} text-white cursor-pointer transition-colors`}>{analizando ? <><Loader2 className="w-5 h-5 animate-spin" /><span className="font-semibold">Procesando...</span></> : <><Upload className="w-5 h-5" /><span className="font-semibold">Subir Cotizaciones (PDFs)</span></>}<input type="file" accept="application/pdf" multiple className="hidden" disabled={analizando} onChange={(e) => { if (e.target.files?.length) handleSubirCotizaciones(e.target.files, cotizacionesModal); e.target.value = ''; }} /></label></div>
            <div className="space-y-3">{(!cotizacionesModal.cotizaciones || !cotizacionesModal.cotizaciones.filter(c => !c.eliminado).length) ? <div className="text-center py-8 text-[var(--fx-muted)]">No hay cotizaciones adjuntas.</div> : cotizacionesModal.cotizaciones.filter(c => !c.eliminado).map((cot, i) => (<div key={i} className="p-4 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10"><div className="flex items-center justify-between mb-2"><div className="flex items-center gap-3"><FileText className="w-5 h-5 text-emerald-400" /><div><div className="text-white font-semibold">{cot.nombre}</div><div className="text-[var(--fx-muted)] text-xs">{new Date(cot.fecha).toLocaleString('es-MX')}</div></div></div><div className="flex items-center gap-2"><a href={cot.url} download={cot.nombre} className="px-3 py-2 rounded-lg bg-blue-500/20 hover:bg-blue-500/30 text-blue-400 flex items-center gap-2 text-sm font-semibold"><Download className="w-4 h-4" />Descargar</a><button onClick={() => handleEliminarCotizacion(cotizacionesModal, cotizacionesModal.cotizaciones?.indexOf(cot) || i)} className="p-2 rounded-lg bg-red-500/20 hover:bg-red-500/30 text-red-400"><Trash2 className="w-4 h-4" /></button></div></div>{cot.analisis && !cot.analisis.error && (<div className="mt-3 p-3 rounded-lg bg-blue-500/10 border border-blue-500/20"><div className="flex items-center gap-2 mb-2"><CheckCircle className="w-4 h-4 text-blue-400" /><span className="text-blue-400 text-xs font-semibold">DATOS EXTRAÍDOS</span></div><div className="grid grid-cols-4 gap-2 text-xs"><div><span className="text-[var(--fx-muted)]">Servicio:</span> <span className="text-white">{cot.analisis.tipoServicio?.join(', ')}</span></div><div><span className="text-[var(--fx-muted)]">Viaje:</span> <span className="text-white">{cot.analisis.tipoViaje?.join(', ')}</span></div><div><span className="text-[var(--fx-muted)]">Rutas:</span> <span className="text-white">{cot.analisis.rutas}</span></div><div><span className="text-[var(--fx-muted)]">Tarifa:</span> <span className="text-emerald-400 font-semibold">${cot.analisis.tarifaMXN?.toLocaleString()} MXN</span></div></div></div>)}</div>))}</div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setEditLead(null)}>
          <div className="bg-[var(--fx-surface)] rounded-2xl border border-white/20 w-[900px] max-h-[90vh] overflow-y-auto p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="p-3 rounded-lg bg-[var(--fx-surface)] border border-white/10"><div className="text-white mb-2 text-xs font-bold">HITOS</div><div className="grid grid-cols-4 gap-2"><label className="flex items-center gap-1 p-2 rounded bg-cyan-500/10 border border-cyan-500/30 cursor-pointer text-xs"><input type="checkbox" checked={formData.altaCliente || false} onChange={(e) => setFormData({ ...formData, altaCliente: e.target.checked })} className="w-3 h-3" /><span className="text-cyan-400">N4</span></label><label className="flex items-center gap-1 p-2 rounded bg-purple-500/10 border border-purple-500/30 cursor-pointer text-xs"><input type="checkbox" checked={formData.generacionSOP || false} onChange={(e) => setFormData({ ...formData, generacionSOP: e.target.checked })} className="w-3 h-3" /><span className="text-purple-400">N5</span></label><label className="flex items-center gap-1 p-2 rounded bg-pink-500/10 border border-pink-500/30 cursor-pointer text-xs"><input type="checkbox" checked={formData.juntaArranque || false} onChange={(e) => setFormData({ ...formData, juntaArranque: e.target.checked })} className="w-3 h-3" /><span className="text-pink-400">N6</span></label><label className="flex items-center gap-1 p-2 rounded bg-yellow-500/10 border border-yellow-500/30 cursor-pointer text-xs"><input type="checkbox" checked={formData.facturado || false} onChange={(e) => setFormData({ ...formData, facturado: e.target.checked })} className="w-3 h-3" /><span className="text-yellow-400">N7</span></label></div></div>
            </div>
