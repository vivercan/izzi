            ...
            <div className="bg-[var(--fx-surface)] rounded-2xl border border-white/20 w-[95vw] max-w-[1400px] max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="p-6">
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <div className="space-y-3">
                    <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                      <div className="text-blue-400 mb-1 flex items-center gap-2" style={{ fontSize: '11px', fontWeight: 600 }}><User className="w-3 h-3" />CONTACTO</div>
                      <div className="text-white font-semibold">{selectedLead.nombreContacto}</div>
                      <div className="text-[var(--fx-muted)] text-sm">{selectedLead.correoElectronico}</div>
                      {selectedLead.telefonoContacto && <div className="text-[var(--fx-muted)] text-sm">{selectedLead.telefonoContacto}</div>}
                    </div>
                    <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                      <div className="text-blue-400 mb-1" style={{ fontSize: '11px', fontWeight: 600 }}>PÁGINA WEB</div>
                      <div className="text-white text-sm">{selectedLead.paginaWeb || 'No especificada'}</div>
                    </div>
                  </div>
                  {/* Col 2: Servicios y Viajes */}
                  <div className="space-y-3">
                    <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                      <div className="text-blue-400 mb-2" style={{ fontSize: '11px', fontWeight: 600 }}>TIPO DE SERVICIO</div>
                      <div className="flex flex-wrap gap-1">{selectedLead.tipoServicio.map((tipo, i) => <span key={i} className="px-2 py-1 rounded bg-blue-500/20 text-blue-400 border border-blue-500/30 text-xs font-semibold">{tipo}</span>)}</div>
                    </div>
                    <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                      <div className="text-green-400 mb-2" style={{ fontSize: '11px', fontWeight: 600 }}>TIPO DE VIAJE</div>
                      <div className="flex flex-wrap gap-1">{selectedLead.tipoViaje.map((tipo, i) => <span key={i} className="px-2 py-1 rounded bg-green-500/20 text-green-400 border border-green-500/30 text-xs font-semibold">{tipo}</span>)}</div>
                    </div>
                    <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                      <div className="text-blue-400 mb-1" style={{ fontSize: '11px', fontWeight: 600 }}>PRÓXIMOS PASOS</div>
                      <div className="text-white text-sm">{selectedLead.proximosPasos || 'Sin próximos pasos'}</div>
                    </div>
                  </div>
                  {/* Col 3: Métricas */}
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-2">
                      <div className="p-3 rounded-xl bg-purple-500/10 border border-purple-500/30">
                        <div className="text-purple-400 mb-1" style={{ fontSize: '10px', fontWeight: 600 }}>RUTAS</div>
                        <div className="text-white font-semibold text-sm">{selectedLead.principalesRutas || '-'}</div>
                      </div>
                      <div className="p-3 rounded-xl bg-orange-500/10 border border-orange-500/30">
                        <div className="text-orange-400 mb-1" style={{ fontSize: '10px', fontWeight: 600 }}>VIAJES/MES</div>
                        <div className="text-white font-bold text-xl" style={{ fontFamily: "'Orbitron', sans-serif" }}>{selectedLead.viajesPorMes || '-'}</div>
                      </div>
                    </div>
                    <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30">
                      <div className="text-emerald-400 mb-1" style={{ fontSize: '10px', fontWeight: 600 }}>TARIFA / VENTA</div>
                      <div className="text-emerald-400 font-bold text-xl" style={{ fontFamily: "'Orbitron', sans-serif" }}>{selectedLead.tarifa || 'N/A'}</div>
                    </div>
                    <div className="p-3 rounded-xl bg-white/5 border border-white/10">
                      <div className="text-yellow-400 mb-2" style={{ fontSize: '11px', fontWeight: 600 }}>HITOS</div>
                      <div className="grid grid-cols-2 gap-1">
                        <div className={`flex items-center gap-1 p-1.5 rounded text-xs ${selectedLead.altaCliente ? 'bg-cyan-500/20 text-cyan-400' : 'bg-gray-500/10 text-gray-500'}`}>{selectedLead.altaCliente ? '✅' : '⬜'} N4</div>
                        <div className={`flex items-center gap-1 p-1.5 rounded text-xs ${selectedLead.generacionSOP ? 'bg-purple-500/20 text-purple-400' : 'bg-gray-500/10 text-gray-500'}`}>{selectedLead.generacionSOP ? '✅' : '⬜'} N5</div>
                        <div className={`flex items-center gap-1 p-1.5 rounded text-xs ${selectedLead.juntaArranque ? 'bg-pink-500/20 text-pink-400' : 'bg-gray-500/10 text-gray-500'}`}>{selectedLead.juntaArranque ? '✅' : '⬜'} N6</div>
                        <div className={`flex items-center gap-1 p-1.5 rounded text-xs ${selectedLead.facturado ? 'bg-yellow-500/20 text-yellow-400' : 'bg-gray-500/10 text-gray-500'}`}>{selectedLead.facturado ? '✅' : '⬜'} N7</div>
                      </div>
                    </div>
                  </div>
                  {/* Col 4: Cotizaciones */}
                  <div className="p-3 rounded-xl bg-emerald-500/5 border border-emerald-500/20">
                    <h4 className="text-emerald-400 flex items-center gap-2 mb-3" style={{ fontSize: '12px', fontWeight: 700 }}><FileText className="w-4 h-4" />COTIZACIONES {selectedLead.cotizaciones?.length ? `(${selectedLead.cotizaciones.length})` : ''}</h4>
                    {(!selectedLead.cotizaciones || selectedLead.cotizaciones.length === 0) ? <div className="text-center py-4 text-[var(--fx-muted)] text-sm">Sin cotizaciones</div> : <div className="space-y-2 max-h-[200px] overflow-y-auto">{selectedLead.cotizaciones.map((cot, i) => <div key={i} className="flex items-center justify-between p-2 rounded-lg bg-white/5 border border-white/10"><div className="flex items-center gap-2 flex-1 min-w-0"><FileText className="w-4 h-4 text-emerald-400 flex-shrink-0" /><div className="truncate text-white text-xs">{cot.nombre}</div></div><a href={cot.url} download={cot.nombre} className="p-1.5 rounded bg-blue-500/20 text-blue-400 flex-shrink-0"><Download className="w-3 h-3" /></a></div>)}</div>}
                  </div>
                </div>
                <div className="flex items-center justify-between pt-4 mt-4 border-t border-white/10">
                  ...
                  <div className="text-[var(--fx-muted)] text-sm"><span className="text-white font-semibold">Capturado:</span> {new Date(selectedLead.fechaCaptura).toLocaleString('es-MX')}</div>
        ...
        {editLead && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setEditLead(null)}>
            <div className="bg-[var(--fx-surface)] rounded-2xl border border-white/20 w-[900px] max-h-[90vh] overflow-y-auto p-6" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-6"><h3 className="text-white flex items-center gap-2" style={{ fontSize: '24px', fontWeight: 700 }}><Pencil className="w-6 h-6 text-yellow-400" />Editar Lead - {editLead.nombreEmpresa}</h3><button onClick={() => setEditLead(null)} className="p-2 rounded-lg hover:bg-white/10"><X className="w-5 h-5 text-white" /></button></div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><div className="text-[var(--fx-muted)] mb-1" style={{ fontSize: '12px' }}>Contacto</div><input type="text" value={formData.nombreContacto || ''} onChange={(e) => handleInputChange('nombreContacto', e.target.value)} className="w-full px-3 py-2 rounded-lg bg-[rgba(15,23,42,0.85)] border border-[rgba(148,163,184,0.4)] text-white focus:outline-none focus:border-[var(--fx-primary)]" style={{ fontSize: '13px' }} /></div>
                <div><div className="text-[var(--fx-muted)] mb-1" style={{ fontSize: '12px' }}>Email</div><input type="email" value={formData.correoElectronico || ''} onChange={(e) => handleInputChange('correoElectronico', e.target.value)} className="w-full px-3 py-2 rounded-lg bg-[rgba(15,23,42,0.85)] border border-[rgba(148,163,184,0.4)] text-white focus:outline-none focus:border-[var(--fx-primary)]" style={{ fontSize: '13px' }} /></div>
                <div className="col-span-2"><div className="text-[var(--fx-muted)] mb-1" style={{ fontSize: '12px' }}>Tipo de Servicio</div><div className="flex flex-wrap gap-2">{['Seco', 'Refrigerado', 'Seco Hazmat', 'Refrigerado Hazmat'].map(s => <button key={s} onClick={() => handleToggleServicio(s)} className={`px-3 py-1.5 rounded-lg ${formData.tipoServicio?.includes(s) ? 'bg-blue-500/30 text-blue-400 border border-blue-500/50' : 'bg-[rgba(15,23,42,0.85)] border border-[rgba(148,163,184,0.4)] text-white'}`} style={{ fontSize: '12px', fontWeight: 600 }}>{s}</button>)}</div></div>
                <div className="col-span-2"><div className="text-[var(--fx-muted)] mb-1" style={{ fontSize: '12px' }}>Tipo de Viaje</div><div className="flex flex-wrap gap-2">{['Impo', 'Expo', 'Nacional', 'Dedicado'].map(v => <button key={v} onClick={() => handleToggleViaje(v)} className={`px-3 py-1.5 rounded-lg ${formData.tipoViaje?.includes(v) ? 'bg-green-500/30 text-green-400 border border-green-500/50' : 'bg-[rgba(15,23,42,0.85)] border border-[rgba(148,163,184,0.4)] text-white'}`} style={{ fontSize: '12px', fontWeight: 600 }}>{v}</button>)}</div></div>
                <div><div className="text-[var(--fx-muted)] mb-1" style={{ fontSize: '12px' }}>Rutas</div><input type="text" value={formData.principalesRutas || ''} onChange={(e) => setFormData({ ...formData, principalesRutas: e.target.value })} className="w-full px-3 py-2 rounded-lg bg-[rgba(15,23,42,0.85)] border border-[rgba(148,163,184,0.4)] text-white focus:outline-none focus:border-[var(--fx-primary)]" style={{ fontSize: '13px' }} /></div>
                <div className="grid grid-cols-2 gap-4">
                  <div><div className="text-[var(--fx-muted)] mb-1" style={{ fontSize: '12px' }}>Viajes/Mes</div><input type="number" value={formData.viajesPorMes || ''} onChange={(e) => setFormData({ ...formData, viajesPorMes: e.target.value })} className="w-full px-3 py-2 rounded-lg bg-[rgba(15,23,42,0.85)] border border-[rgba(148,163,184,0.4)] text-white focus:outline-none focus:border-[var(--fx-primary)]" style={{ fontSize: '13px' }} /></div>
                  <div><div className="text-[var(--fx-muted)] mb-1" style={{ fontSize: '12px' }}>Tarifa</div><input type="text" value={formData.tarifa || ''} onChange={(e) => setFormData({ ...formData, tarifa: e.target.value })} className="w-full px-3 py-2 rounded-lg bg-[rgba(15,23,42,0.85)] border border-[rgba(148,163,184,0.4)] text-white focus:outline-none focus:border-[var(--fx-primary)]" style={{ fontSize: '13px' }} /></div>
                </div>
                <div className="col-span-2"><div className="text-[var(--fx-muted)] mb-1" style={{ fontSize: '12px' }}>Próximos Pasos</div><textarea value={formData.proximosPasos || ''} onChange={(e) => setFormData({ ...formData, proximosPasos: e.target.value })} rows={2} className="w-full px-3 py-2 rounded-lg bg-[rgba(15,23,42,0.85)] border border-[rgba(148,163,184,0.4)] text-white focus:outline-none focus:border-[var(--fx-primary)]" style={{ fontSize: '13px' }} /></div>
                ...
                <div className="p-3 rounded-lg bg-[var(--fx-surface)] border border-white/10">
                  <div className="text-white mb-2" style={{ fontSize: '12px', fontWeight: 700 }}>HITOS DEL CLIENTE</div>
