...
async function getDocxXml(file: File): Promise<string> {
...
function parseDocxXml(xml: string): { text: string; html: string } {
  if (!xml) return { text: '', html: '' };
  const paragraphs = xml.split('</w:p>');
  let text = '', html = '';
  for (const para of paragraphs) {
    const isCenter = /w:jc\s+w:val\s*=\s*"center"/.test(para);
    const pprMatch = para.match(/<w:pPr>([\s\S]*?)<\/w:pPr>/);
    let pprBold = false;
    if (pprMatch) {
      const rprInPpr = pprMatch[1].match(/<w:rPr>([\s\S]*?)<\/w:rPr>/);
      if (rprInPpr && /<w:b[\s\/]/.test(rprInPpr[1]) && !/<w:b\s+w:val\s*=\s*"(false|0)"/.test(rprInPpr[1])) pprBold = true;
    }
    const runRx = /<w:r\b[\s\S]*?<\/w:r>/g;
    const runs: { txt: string; bold: boolean; italic: boolean; underline: boolean }[] = [];
    let rm;
    while ((rm = runRx.exec(para)) !== null) {
      ...
      if (!runTxt) continue;
      const runBold = /<w:b[\s\/]/.test(run) && !/<w:b\s+w:val\s*=\s*"(false|0)"/.test(run);
      const runItalic = /<w:i[\s\/]/.test(run) && !/<w:i\s+w:val\s*=\s*"(false|0)"/.test(run);
      const runUnder = /<w:u[\s\/]/.test(run) && !/<w:u\s+w:val\s*=\s*"none"/.test(run);
      runs.push({ txt: runTxt, bold: runBold || pprBold, italic: runItalic, underline: runUnder });
    }
    if (runs.length === 0) { html += '<p style="margin:4px 0;line-height:0.6">&nbsp;</p>\n'; text += '\n'; continue; }
    ...
    if (allBold) inline = '<strong>' + inline + '</strong>';
    let style = 'margin:2px 0;line-height:1.7;font-size:11pt;';
    if (isCenter) style += 'text-align:center;';
    else style += 'text-align:justify;';
    if (/^\s*[a-h]\)/.test(fullText)) style += 'margin-left:30px;';
    if (isCenter && allBold) style += 'font-size:13pt;margin:20px 0 10px;letter-spacing:0.5px;';
    else if (allBold && /^(PRIMERA|SEGUNDA|TERCERA|CUARTA|QUINTA|SEXTA|S\u00c9PTIMA|OCTAVA|NOVENA|D\u00c9CIM|UND\u00c9CIM|DUOD\u00c9CIM|I{1,3}\.|IV\.|V\.|VI|VII|VIII|IX|X|CONTRATO DE)/.test(fullText.trim())) style += 'margin:14px 0 4px;';
    html += '<p style="' + style + '">' + inline + '</p>\n';
  }
  text = text.trim().replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"').replace(/&apos;/g, "'");
  return { text, html };
}

function formatPlainTextAsLegalHtml(text: string): string {
  if (!text) return '';
  const lines = text.split('\n');
  let html = '';
  for (const line of lines) {
    const t = line.trim();
    if (!t) { html += '<p style="margin:4px 0;line-height:0.6">&nbsp;</p>\n'; continue; }
    let style = 'margin:2px 0;line-height:1.7;font-size:11pt;text-align:justify;';
    let content = esc(t);
    if (/^(DECLARACIONES|CL\u00c1USULAS|ANEXO|FIRMAS):?\s*$/.test(t)) {
      style = 'margin:20px 0 10px;line-height:1.7;font-size:13pt;text-align:center;letter-spacing:0.5px;';
      content = '<strong>' + content + '</strong>';
    } else if (/^(PRIMERA|SEGUNDA|TERCERA|CUARTA|QUINTA|SEXTA|S\u00c9PTIMA|OCTAVA|NOVENA|D\u00c9CIM)/.test(t)) {
      style += 'margin:14px 0 4px;';
      content = '<strong>' + content + '</strong>';
    } else if (/^(I{1,3}|IV|V|VI{0,3}|IX|X)\.\s+(DECLAR)/.test(t)) {
      style += 'margin:14px 0 4px;';
      content = '<strong>' + content + '</strong>';
    } else if (/^[a-h]\)\s/.test(t)) {
      style += 'margin-left:30px;';
    } else if (/^\d+[\.\)]\s/.test(t)) {
      style += 'margin-left:20px;';
    } else if (t.length > 50 && t === t.toUpperCase() && /^CONTRATO/.test(t)) {
      style += 'margin:10px 0;';
      content = '<strong>' + content + '</strong>';
    }
...
async function extractDocxFormatted(file: File): Promise<{ text: string; html: string }> {
  try {
    const xml = await getDocxXml(file);
    if (xml) return parseDocxXml(xml);
    return { text: '', html: '' };
  } catch (e) { console.error('DOCX extraction:', e); return { text: '', html: '' }; }
}

const PDF_CSS = '@page{margin:2cm 2.5cm;size:letter}body{font-family:"Merriweather",Georgia,serif;font-size:11pt;line-height:1.7;color:#1e293b;margin:0;padding:30px 40px}h1{font-family:"Exo 2",Helvetica,sans-serif;color:#0c4a6e;font-size:20pt;border-bottom:3px solid #0284c7;padding-bottom:10px;margin-bottom:20px}h2{font-family:"Exo 2",Helvetica,sans-serif;color:#0c4a6e;font-size:14pt;margin-top:24px;border-left:4px solid #0284c7;padding-left:10px}h3{font-family:"Exo 2",Helvetica,sans-serif;color:#1e3a5f;font-size:12pt;margin:12px 0 6px}.meta{font-family:"Exo 2",sans-serif;font-size:9pt;color:#777;margin-bottom:16px}.badge{display:inline-block;padding:3px 12px;border-radius:16px;font-family:"Exo 2",sans-serif;font-weight:700;font-size:10pt}.b-alta{background:#fee2e2;color:#dc2626}.b-media{background:#fef3c7;color:#d97706}.b-baja{background:#d1fae5;color:#059669}.risk-box{border-left:5px solid;padding:12px 16px;margin:12px 0;border-radius:0 8px 8px 0}.warn{background:#fef2f2;border:2px solid #ef4444;padding:14px;border-radius:8px;margin:14px 0}.ok-box{background:#f0fdf4;border:2px solid #22c55e;padding:14px;border-radius:8px;margin:14px 0}table{width:100%;border-collapse:collapse;margin:14px 0}th,td{padding:8px 12px;border:1px solid #d1d5db;text-align:left;font-size:10pt}th{background:#0c4a6e;color:#fff;font-family:"Exo 2",sans-serif}.contract-body{font-family:"Merriweather",Georgia,serif;font-size:11pt;line-height:1.7;text-align:justify}.contract-body p{margin:2px 0}.footer{margin-top:30px;padding-top:12px;border-top:2px solid #e5e7eb;font-family:"Exo 2",sans-serif;font-size:8pt;color:#aaa;text-align:center}.page-break{page-break-before:always}';

function generarPDF(titulo: string, html: string) {
  const w = window.open('', '_blank');
  if (!w) { alert('Permite ventanas emergentes'); return; }
  w.document.write('<!DOCTYPE html><html><head><meta charset="utf-8"><title>' + titulo + '</title><link href="https://fonts.googleapis.com/css2?family=Merriweather:wght@300;400;700&family=Exo+2:wght@400;600;700;900&display=swap" rel="stylesheet"><style>' + PDF_CSS + '</style></head><body>' + html + '<div class="footer">FX27 — TROB TRANSPORTES S.A. DE C.V. | ' + new Date().toLocaleDateString('es-MX',{day:'2-digit',month:'long',year:'numeric'}) + '</div></body></html>');
...
function riskSection(riesgos: RiesgoContrato[], faltantes: string[]) {
  ...
  riesgos.forEach((r, i) => {
    ...
    h += '<div class="risk-box" style="border-color:' + bc + ';background:' + bg + '"><h3>' + (i+1) + '. ' + r.clausula + ' <span class="badge b-' + r.severidad.toLowerCase() + '">' + r.severidad + '</span></h3>';
    if (r.texto_original) h += '<div style="padding:8px 12px;background:rgba(0,0,0,0.04);border-radius:4px;margin:6px 0"><p style="margin:0;font-size:9pt;color:#999;font-weight:700">❌ DICE ACTUALMENTE:</p><p style="margin:3px 0 0;font-size:10pt;font-style:italic">"' + r.texto_original + '"</p></div>';
...
function verdictPage(r: AnalisisContrato, f: string) {
  ...
  const border = firmar ? '#22c55e' : '#dc2626'; const bg = firmar ? '#f0fdf4' : '#fef2f2';
  ...
  const txtC = firmar ? '#16a34a' : '#dc2626'; const subC = firmar ? '#166534' : '#991b1b';
  const bodyC = firmar ? '#14532d' : '#7f1d1d'; const btnBg = firmar ? '#16a34a' : '#dc2626';
  ...
  return '<div class="page-break" style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:80vh;text-align:center"><div style="border:8px solid ' + border + ';border-radius:20px;padding:60px 80px;background:' + bg + '"><p style="font-family:\'Exo 2\',sans-serif;font-size:56pt;font-weight:900;color:' + txtC + ';margin:0;line-height:1.1">' + emoji + '</p><div style="width:100%;height:4px;background:' + border + ';margin:30px 0"></div><p style="font-family:\'Exo 2\',sans-serif;font-size:16pt;color:' + subC + ';margin:10px 0;font-weight:700">' + sub + '</p>' + (!firmar && r.es_leonino ? '<p style="font-family:\'Exo 2\',sans-serif;font-size:20pt;color:#dc2626;margin:16px 0;font-weight:900">🚨 CONTRATO LEONINO 🚨</p>' : '') + '<p style="font-size:12pt;color:' + bodyC + ';margin:20px 0;line-height:1.8;max-width:600px">' + (r.justificacion_veredicto || (firmar ? 'Condiciones equilibradas y razonables.' : 'Condiciones desfavorables que requieren corrección.')) + '</p><div style="margin-top:30px;padding:14px 20px;background:' + btnBg + ';border-radius:10px"><p style="font-family:\'Exo 2\',sans-serif;font-size:13pt;color:#fff;margin:0;font-weight:700">' + btnTxt + '</p></div><p style="font-size:9pt;color:#aaa;margin:24px 0 0">Calificación: ' + r.calificacion_riesgo + '/10 | TROB TRANSPORTES | ' + f + '</p></div></div>';
}

const PASOS = [
  { texto: 'Preparando documento para análisis', pct: 8 }, { texto: 'Extrayendo texto y estructura', pct: 20 },
  { texto: 'Identificando partes del contrato', pct: 35 }, { texto: 'Analizando cláusulas y condiciones', pct: 52 },
...
export default function AnalisisContratosModule({ onBack }: Props) {
  ...
  const handleFile = async (file: File) => {
    ...
    setArchivo(file); setError(''); setResultado(null); setTextoExtraido(''); setTextoHtml('');
    const reader = new FileReader();
    reader.onload = () => setArchivoBase64((reader.result as string).split(',')[1]);
    reader.readAsDataURL(file);
    if (ext === 'docx') {
      try {
        const { text, html } = await extractDocxFormatted(file);
        if (text && text.length > 30) { setTextoExtraido(text); console.log('Text: ' + text.length + ' chars'); }
        if (html && html.length > 30) { setTextoHtml(html); console.log('HTML: ' + html.length + ' chars'); }
  ...
  const getContractHtml = (): string => {
    if (textoHtml) return textoHtml;
    if (textoExtraido) return formatPlainTextAsLegalHtml(textoExtraido);
    return '<p style="color:#999;font-style:italic">(Texto original disponible en archivo fuente)</p>';
  };

  const descargarContratoCompleto = () => {
    if (!resultado) return; const f = fecha();
    let h = '<div style="text-align:center;margin-bottom:30px"><p style="font-family:\'Exo 2\',sans-serif;font-size:9pt;color:#666;letter-spacing:3px;text-transform:uppercase;margin:0">TROB TRANSPORTES S.A. DE C.V.</p><h1 style="border-bottom:none;margin:6px 0">CONTRATO</h1><p style="font-size:10pt;color:#666">' + (archivo?.name || '') + ' | ' + f + '</p><div style="width:80px;height:3px;background:#0284c7;margin:10px auto"></div></div>';
    h += contratoTable(resultado.datos_extraidos) + trobTable();
    h += '<div class="page-break"></div><div class="contract-body">' + getContractHtml() + '</div>';
    if (resultado.riesgos.length > 0 || resultado.es_leonino) {
      ...
      h += '<h2>RIESGOS IDENTIFICADOS (' + resultado.riesgos.length + ')</h2>' + riskSection(resultado.riesgos, resultado.clausulas_faltantes);
    }
  ...
  const descargarBlindada = () => {
    if (!resultado) return; const f = fecha();
    const blindadaHtml = resultado.version_blindada ? formatPlainTextAsLegalHtml(resultado.version_blindada) : '<p>(Versión blindada no disponible)</p>';
    let h = '<div style="text-align:center;margin-bottom:30px"><p style="font-family:\'Exo 2\',sans-serif;font-size:9pt;color:#059669;letter-spacing:3px;text-transform:uppercase;margin:0">VERSIÓN BLINDADA — PROTECCIÓN TOTAL TROB</p><h1 style="border-bottom-color:#22c55e;margin:6px 0">CONTRATO CORREGIDO</h1><p style="font-size:10pt;color:#666">Basado en: ' + (archivo?.name || '') + ' | ' + f + '</p><div style="width:80px;height:3px;background:#22c55e;margin:10px auto"></div></div>';
    ...
    h += contratoTable(resultado.datos_extraidos) + trobTable();
    h += '<div class="page-break"></div><div class="contract-body">' + blindadaHtml + '</div>';
    if (resultado.riesgos.length > 0) {
      h += '<div class="page-break"></div><h1 style="color:#0c4a6e">REGISTRO DE CORRECCIONES</h1><div class="meta">Cambios aplicados | ' + f + '</div>';
      resultado.riesgos.forEach((r, i) => {
        h += '<div style="border-left:5px solid #0284c7;padding:12px 16px;margin:12px 0;background:#f0f9ff;border-radius:0 8px 8px 0"><h3>' + (i+1) + '. ' + r.clausula + ' <span class="badge b-' + r.severidad.toLowerCase() + '">' + r.severidad + '</span></h3>';
        ...
        h += '<div style="padding:8px 12px;background:rgba(34,197,94,0.06);border-left:3px solid #22c55e;border-radius:4px;margin:6px 0"><p style="margin:0;font-size:9pt;color:#059669;font-weight:700">DESPUÉS:</p><p style="margin:3px 0 0;font-size:10pt">' + r.sugerencia + '</p></div>';
        h += '<p style="font-size:9pt;color:#64748b;margin:4px 0 0"><strong>Motivo:</strong> ' + r.descripcion + '</p></div>';
      });
    ...
    h += '<div class="page-break" style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:80vh;text-align:center"><div style="border:8px solid #22c55e;border-radius:20px;padding:60px 80px;background:#f0fdf4"><p style="font-family:\'Exo 2\',sans-serif;font-size:50pt;font-weight:900;color:#16a34a;margin:0;line-height:1.1">✅ APTO PARA FIRMA</p><div style="width:100%;height:4px;background:#22c55e;margin:30px 0"></div><p style="font-family:\'Exo 2\',sans-serif;font-size:16pt;color:#166534;margin:10px 0;font-weight:700">VERSIÓN BLINDADA — CORRECCIONES APLICADAS</p><p style="font-size:12pt;color:#14532d;margin:20px 0;line-height:1.8;max-width:600px">Se corrigieron ' + resultado.riesgos.length + ' punto' + (resultado.riesgos.length!==1?'s':'') + ' de riesgo' + (resultado.clausulas_faltantes.length ? ' y se agregaron ' + resultado.clausulas_faltantes.length + ' cláusula' + (resultado.clausulas_faltantes.length!==1?'s':'') + ' faltante' + (resultado.clausulas_faltantes.length!==1?'s':'') : '') + '.</p><div style="margin-top:30px;padding:14px 20px;background:#16a34a;border-radius:10px"><p style="font-family:\'Exo 2\',sans-serif;font-size:13pt;color:#fff;margin:0;font-weight:700">Proceder con la firma previa validación del equipo legal de TROB.</p></div><p style="font-size:9pt;color:#aaa;margin:24px 0 0">Riesgo original: ' + resultado.calificacion_riesgo + '/10 (corregido) | TROB TRANSPORTES | ' + f + '</p></div></div>';
  ...
  const descargarRiesgos = () => {
    ...
    let h = '<h1>⚠️ Análisis de Riesgos</h1><div class="meta">' + (archivo?.name || '') + ' | ' + f + '</div><div style="text-align:center;margin:20px 0"><span class="badge b-' + rg.label.toLowerCase() + '" style="font-size:16pt">' + resultado.calificacion_riesgo + '/10 — RIESGO ' + rg.label + '</span></div>';
    if (resultado.es_leonino) h += '<div class="warn"><strong>⚠️ CONTRATO LEONINO</strong><p>' + resultado.explicacion_leonino + '</p></div>';
    h += '<h2>Resumen Ejecutivo</h2><p style="text-align:justify;line-height:1.8">' + resultado.resumen_ejecutivo + '</p>';
    ...
    h += '<h2>Detalle</h2>' + riskSection([...a,...m,...b], resultado.clausulas_faltantes) + verdictPage(resultado, f);
        ...
        {!resultado && (
          <div className="space-y-5">
            <div onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)}
              ...
              style={{ background: dragOver ? 'linear-gradient(135deg, rgba(14,165,233,0.12), rgba(6,78,59,0.08))' : 'linear-gradient(135deg, rgba(15,23,42,0.90), rgba(30,41,59,0.70))', border: dragOver ? '2px dashed #0ea5e9' : '2px dashed rgba(100,116,139,0.25)', minHeight: '200px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 16px rgba(0,0,0,0.15)' }}>
              ...
              {archivo ? (<>
                ...
                <p style={{ fontFamily: F, fontSize: '13px', color: '#64748b', marginTop: '4px' }}>{(archivo.size / (1024 * 1024)).toFixed(2)} MB{textoExtraido ? ' · ' + textoExtraido.length + ' caracteres' : ''}{textoHtml ? ' · formato OK' : ''}</p>
                <button onClick={e => { e.stopPropagation(); setArchivo(null); setArchivoBase64(''); setTextoExtraido(''); setTextoHtml(''); setError(''); }}
              ...
              return (<div className="rounded-xl p-5" style={{ background: 'linear-gradient(135deg, ' + rg.bg + ', rgba(15,23,42,0.90))', border: '1px solid ' + rg.color + '25', boxShadow: '0 4px 20px rgba(0,0,0,0.20)' }}>
                <div className="flex items-center justify-between flex-wrap gap-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button onClick={() => { setResultado(null); setArchivo(null); setArchivoBase64(''); setTextoExtraido(''); setTextoHtml(''); setError(''); setProgreso(0); }}
