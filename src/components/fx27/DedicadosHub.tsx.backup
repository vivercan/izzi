...
export const DedicadosHub = ({ onBack, onNavigate }: DedicadosHubProps) => {
  ...
  return (
    <div 
      ...
      style={{
        background: 'linear-gradient(180deg, #E6ECF2 0%, #D8DFE8 50%, #E6ECF2 100%)',
        padding: '32px 64px'
      }}
    >
      ...
      <button 
        onClick={onBack} 
        className="fixed top-8 left-8 z-10 flex items-center justify-center transition-all duration-200"
        style={{
          width: '48px',
          height: '48px',
          borderRadius: '10px',
