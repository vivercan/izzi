        ...
        <div style={{ padding: '10px 32px', flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          ...
          <div style={{ display: 'flex', gap: '10px', marginBottom: '10px', flexShrink: 0 }}>
            <button onClick={() => setCxcFilterAgente('TODOS')} style={{
              padding: '12px 18px', borderRadius: '10px', cursor: 'pointer', fontSize: '14px', fontWeight: 700, fontFamily: "'Exo 2', sans-serif",
            ...
            {ejecCards.map(ec => (
              <button key={ec.agente} onClick={() => setCxcFilterAgente(cxcFilterAgente === ec.agente ? 'TODOS' : ec.agente)} style={{
                ...
                <div style={{ textAlign: 'left', flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: '13px', fontWeight: 700, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{ec.nombre}</div>
                  <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)', marginTop: '2px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{ec.clientes} clientes · {fmt(ec.saldo)}{ec.vencido > 0 ? ` · ${fmt(ec.vencido)} venc.` : ''}</div>
                </div>
