...
export default function DespachoInteligenteContent() {
  ...
  const [empF, setEmpF] = useState(['SHI', 'TROB', 'WE']);
  ...
  const stats = { total: fleet.length, mov: fleet.filter(u => u.status === 'moving').length, det: fleet.filter(u => u.status === 'stopped').length, sin: fleet.filter(u => u.status === 'no_signal').length, emp: { SHI: fleet.filter(u => u.empresa === 'SHI').length, TROB: fleet.filter(u => u.empresa === 'TROB').length, WE: fleet.filter(u => u.empresa === 'WE').length } };
